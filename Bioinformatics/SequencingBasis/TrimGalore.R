###################################################################################
# 函数功能简介：使用trim galore进行fastq文件质量控制
...
#### 文件格式要求
#' 要求为pattern前必须有_1/2，否则文件匹配不到会出错
#### 接头处理
...
trimGalore <- function(fastqDir, outDir, pairEND, is.phred33Encoding = TRUE, pattern = "fastq.gz", q = 20, length = 20, threads = 20, extraParameter = NULL){
  
  # path_to_cutadapt <- "/pub5/xiaoyun/BioSoftware/cutadapt/exe/bin/cutadapt"
  # 导入指定的环境变量
  # system("export PATH=$PATH:/pub5/xiaoyun/BioSoftware/cutadapt/exe/bin")
  # system("export PYTHONPATH=$PYTHONPATH:/pub5/xiaoyun/BioSoftware/cutadapt/exe/lib/python2.7:/pub5/xiaoyun/BioSoftware/cutadapt/exe/lib/python2.7/site-packages")
  
  # 使用GNU Parallel工具来实现并行
  # -j 表示并行jobs的数量
  parallel_trimGalore <- paste("/pub5/xiaoyun/BioSoftware/parallel/bin/parallel", "-j", threads, "/pub5/xiaoyun/BioSoftware/trim_galore/trim_galore", sep = " ")
  ...
  if(is.phred33Encoding){
    parameterSet <- paste(" --phred33", "-q", q, "--length", length, "--no_report_file", sep = " ")
  }else{
    parameterSet <- paste(" --phred64", "-q", q, "--length", length, "--no_report_file", sep = " ")
  }
